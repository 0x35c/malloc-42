...
int init_allocator(void)
{
	...
	Zone *tiny = (Zone *)mmap(
	    NULL, tiny_size * NB_BLOCKS_INIT + sizeof(Zone),
	...
	// Init SMALL zones with NB_BLOCKS_INIT blocks of 16 pages, and set the
	// TINY to point towards SMALL
	Zone *small = (Zone *)mmap(
	    NULL, small_size * NB_BLOCKS_INIT + sizeof(Zone),
	    PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
	if (small == NULL) {
		ft_dprintf(2, "error on syscall mmap\n");
		return (-1);
